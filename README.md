...
- **Инициализация чата на клиенте.** При маунте — сначала имитация авторизации (`GET /api/v1/init`, только если в `localStorage` ещё нет токена): получает временный токен и `chatUid` (последний использованный чат или новый), затем отдельно `GET /messages/list` (история) и отдельно `GET /messages/resume` (SSE, проверка активной генерации), как того требует раздел 7.5 ТЗ.
...
- **Новый чат.** Очищает текущий `chatUid` (в т.ч. в `localStorage`) — сервер сам генерирует id при отправке первого сообщения нового чата (chatUid никогда не генерируется на клиенте) и возвращает его клиенту в `messageMetadata` `start`-чанка ответа ассистента; клиент подхватывает id оттуда.
...
- **История чатов.** Кнопка "History" рядом с "New chat" показывает список всех чатов текущего токена (`GET /chats/list`), сгруппированный по дням ("Today" / "Yesterday" / `DD.MM.YYYY`); клик по чату открывает его, кнопка "← Back" возвращает к текущему чату без переключения. Каждый чат можно переименовать (`POST /chats/rename`, инлайн-редактирование) или удалить (`POST /chats/delete`) прямо из списка.

## Известные ограничения (сознательно, т.к. это прототип)

- Один активный "пользователь" на инстанс; `GET /api/v1/init` — это имитация авторизации (минтит токен, ни с чем реально не сверяя), а не настоящий логин.
